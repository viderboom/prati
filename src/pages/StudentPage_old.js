...
export default function StudentPage() {
	...
	useEffect(() => {
		const fetchSessions = async () => {
			...
			try {
				...
				// Map docs to include both `id` and document data
	...
	// Handle marking a session as paid using the custom confirmation modal
	const handleMarkAsPaid = async (sessionId, isGroup) => {
		...
		try {
			...
			await updateDoc(sessionRef, { paid: true }) // Set the paid status to true
	...
	// Handle canceling a session using the custom confirmation modal
	...
	// Helper function to convert month index to Hebrew month name
	...
	const groupedSessions = sessions.reduce((acc, session) => {
		...
		const monthYear = `${year}-${month}` // Unique key for each month-year combination
